...
export interface MagicDustProps {
  ...
  /** Width of the text plane in 3D units (bigger = larger text on screen) */
  textSize?: number;
  /** Scale the text to always span the viewport width (small side padding) */
  fillWidth?: boolean;
  /** When false, the sequence plays once and freezes on the last item */
  loop?: boolean;
  /** Extra spacing between letters, in px of the 1024px text canvas */
  letterSpacing?: number;
}

// Session-level memory: with loop=false the intro plays once per page load,
// not again when navigating back to the page that hosts it.
let playedOnceThisSession = false;
...
function getTextPositions(
  ...
  // measureText may ignore letterSpacing (Chromium), so measure the glyphs
  // alone and add the spacing manually if the browser didn't count it.
  ctx.letterSpacing = '0px';
  const baseWidth = ctx.measureText(text).width;
  ctx.letterSpacing = `${letterSpacing}px`;
  const spacedWidth = ctx.measureText(text).width;
  const spacingExtra =
    spacedWidth > baseWidth
      ? spacedWidth - baseWidth
      : letterSpacing * Math.max(0, text.length - 1);

  const maxWidth = 880; // keep a margin inside the 1024px canvas
  if (baseWidth + spacingExtra > maxWidth) {
    fontSize = Math.max(10, Math.floor(fontSize * ((maxWidth - spacingExtra) / baseWidth)));
    ctx.font = `900 ${fontSize}px ${fontFamily}`;
    ctx.letterSpacing = `${letterSpacing}px`; // some engines reset it with font
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Tighter jitter than the original (0.15/0.2) so glyph edges stay crisp
    positions[i * 3] = p.x + (Math.random() - 0.5) * 0.06;
    positions[i * 3 + 1] = p.y + (Math.random() - 0.5) * 0.06;
...
export function MagicDustCore({
  ...
  const [{ origin, targets, sizes }] = useState(() => {
    ...
    for (const item of sequence) {
      ...
      if (item.type === 'text') {
        dest = getTextPositions(item.text, particleCount, textSize, fontFamily, letterSpacing);
        isText = true;
      } else {
  ...
  // If the play-once intro already ran this session, mount directly settled
  const [startSettled] = useState(() => !loop && playedOnceThisSession);
  useEffect(() => {
    if (!loop) playedOnceThisSession = true;
  }, [loop]);

  const currentProgress = useRef(0);
  const targetProgress = useRef(0);
  const currentTargetIndex = useRef(startSettled ? targets.length - 1 : 0);

  // DONE = play-once sequence finished; nothing renders until a page reload
  const phase = useRef<'CONSTRUCTING' | 'HOLDING' | 'DECONSTRUCTING' | 'DONE'>(
    startSettled ? 'DONE' : 'CONSTRUCTING'
  );
  ...
  useFrame((state, delta) => {
    if (phase.current === 'CONSTRUCTING') {
      ...
      if (targetProgress.current === 0.0) {
        if (!loop && currentTargetIndex.current === targets.length - 1) {
          // Play-once: after the last item dissolves, stay empty until reload
          phase.current = 'DONE';
        } else {
    ...
    if (pointsGroupRef.current) {
      // Responsive: never overflow the viewport, based on 3D units.
      // fillWidth stretches the text plane to span the viewport instead of capping at 1.
      const maxComponentWidth = textSize + 2.0;
      const rawScale = state.viewport.width / maxComponentWidth;
      const scale = fillWidth ? rawScale : Math.min(1.0, rawScale);
      ...
      if (phase.current === 'DONE') {
        // Sequence finished — particles are invisible, stop all motion
        pointsGroupRef.current.rotation.x += (0 - pointsGroupRef.current.rotation.x) * 0.05;
      } else {
    ...
    <points ref={pointsGroupRef}>
      <bufferGeometry ref={geoRef}>
        ...
        <bufferAttribute
          ...
          args={[targets[startSettled ? targets.length - 1 : 0].dest, 3]}
        />
        <bufferAttribute
          attach='attributes-aDelay'
          args={[targets[startSettled ? targets.length - 1 : 0].delays, 1]}
        />
      ...
      <shaderMaterial
        ...
        transparent={true}
        depthWrite={false}
        // Original uses AdditiveBlending (white dust on black). Additive light
        // is invisible over a white page, so dark particles need normal blending.
