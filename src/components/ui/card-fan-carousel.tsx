...
// 21st.dev "Card Fan Carousel" by Aayush Duhan (from grid2.md).
// Adapted: arrows get a solid white background, pagination arrows/dots show
// for any count > 1 (original hid them below 8 cards), and the .fan-card /
// .fan-layout sizing CSS is provided as Tailwind classes.
...
// Scales the horizontal fan spread. Tuned so the outermost cards (bigger
// square cards, rotated ±21°) stay inside the viewport at every width.
function getResponsiveMultiplier(width: number) {
  if (width < 480) return 0.18;
  if (width < 640) return 0.25;
  if (width < 768) return 0.38;
  if (width < 1024) return 0.45;
  if (width < 1280) return 0.6;
...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border-[1.5px] border-black/10 bg-white text-black/40 cursor-pointer shrink-0 z-30 outline-none shadow-[0_4px_20px_rgba(0,0,0,0.1)] hover:border-black/25 hover:text-black/70 active:opacity-70 transition-colors duration-300 before:content-[''] before:absolute before:inset-[3px] before:rounded-full before:border before:border-black/[0.04] before:pointer-events-none";

const CARD_CLASSES =
  'fan-card absolute w-[11rem] h-[11rem] min-[480px]:w-[14rem] min-[480px]:h-[14rem] min-[640px]:w-[16rem] min-[640px]:h-[16rem] min-[768px]:w-[20rem] min-[768px]:h-[20rem] min-[1024px]:w-[26rem] min-[1024px]:h-[26rem] min-[1536px]:w-[28rem] min-[1536px]:h-[28rem] overflow-hidden rounded-[24px] border border-black/5 bg-neutral-100 shadow-[0_10px_40px_rgba(0,0,0,0.18)] will-change-transform';
...
export default function SocialCards({ cards, onCardClick }: SocialCardsProps) {
  ...
  const overflows = totalCards > MAX_VISIBLE;
  const showPagination = totalCards > 1;
  const [centerIndex, setCenterIndex] = useState(overflows ? HALF : totalCards >> 1);

  const getVisibleMap = useCallback(
    (center: number) => {
      const map = new Map<number, number>();
      if (!overflows) {
        // All cards visible — rotate which one sits in the center slot.
        const centerSlot = totalCards >> 1;
        cards.forEach((_, i) => {
          map.set(i, (((i - center + centerSlot) % totalCards) + totalCards) % totalCards);
        });
        return map;
      }
  ...
  useEffect(() => {
    ...
    const slotCount = overflows ? MAX_VISIBLE : totalCards;
  ...
  }, [centerIndex, totalCards, getVisibleMap, overflows]);
          ...
          {cards.map((card, index) => {
            const image = (
              <div className='relative w-full h-full overflow-hidden'>
                ...
                />
                {card.title && (
                  <div className='absolute inset-x-0 bottom-0 z-20 bg-gradient-to-t from-black/60 to-transparent px-4 pt-10 pb-4'>
            ...
            if (onCardClick) {
              return (
                <div
                  ...
                  role='button'
                  tabIndex={0}
                  aria-label={card.title || card.alt || `Card ${index + 1}`}
            ...
            {cards.map((_, i) => (
              <span
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  i === centerIndex ? 'bg-black/70 scale-[1.3]' : 'bg-black/15'
                }`}
              />
            ))}
          </div>
